...
for code,name in WATCHLIST:
    try:
        ...
        # V31 只請求1次60m含盤前後，然後過濾出正股時段，避免限流和nan
        df_60m_all, _ = get_history(code,"60d","60m",True)
        if df_60m_all.empty:
            df_60m_all = df_day  # fallback
            df_60m_reg = df_day
        else:
            # 過濾正股時段 09:30-16:00，美東時間
            try:
                # yfinance index已經是美東
        ...
        h_close_t = d_close_t  # 今的時k收盤用日收，盤後不變灰
...
.section{margin-top:18px} .section-title{font-size:13px;color:#c4b5fd;margin:0 0 8px;padding-left:10px;border-left:4px solid #a855f7;position:relative}
.section-title::before{content:'';position:absolute;left:-4px;top:0;bottom:0;width:4px;background:linear-gradient(180deg,#ec4899,#8b5cf6,#3b82f6);border-radius:2px;opacity:0.8}
...
<div class="header">
<h1><a href="https://hk.finance.yahoo.com/markets" target="_blank" style="text-decoration:none" title="Yahoo HK">🚀</a> MA(20/60/240) 私有版 <span class="ver">V31</span></h1>
...
<div class="section"><div class="section-title">默認區</div><div class="grid" id="grid">__CARDS__</div></div>
...
function getCustomKey(){ var oldKeys = ["customStocksV30","customStocksV29","customStocksV28","customStocksV27","customStocksV26"]; for(var i=0;i<oldKeys.length;i++){ var d = localStorage.getItem(oldKeys[i]); if(d && !localStorage.getItem(CUSTOM_KEY)){ localStorage.setItem(CUSTOM_KEY, d); break; } } return CUSTOM_KEY; }
...
print(f"V31 生成完成 修復nan灰色問題 限流1次請求 紫色彩虹標題 默認區加標")
