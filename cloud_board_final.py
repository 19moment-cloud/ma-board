...
.header{{display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin:10px 0}} h1{{font-size:18px;margin:0}}
.search{{flex:1;min-width:180px;max-width:260px;position:relative}} .addbox{{flex:1;min-width:260px;max-width:400px;display:flex;gap:6px}}
.search input, .addbox input{{width:100%;padding:10px 14px 10px 36px;border-radius:12px;border:1px solid #334155;background:#1e293b;color:#fff;font-size:12px;outline:none}}
.addbox input{{padding-left:14px}} .search input:focus, .addbox input:focus{{border-color:#f59e0b}} .search .icon{{position:absolute;left:10px;top:50%;transform:translateY(-50%)}}
.addbtn{{padding:10px 16px;border-radius:12px;border:none;background:#f59e0b;color:#000;font-weight:800;cursor:pointer;white-space:nowrap}}
...
.card{{border-radius:14px;padding:11px 13px;display:flex;justify-content:space-between;align-items:flex-start;gap:10px;min-height:110px;overflow:hidden;transition:all 0.2s}} .card.hide{{display:none}}
.橙{{background:linear-gradient(135deg,#ff8a00,#ffb347);color:#000}} .綠{{background:linear-gradient(135deg,#00c853,#69f0ae);color:#000}}
.灰{{background:#1e293b;color:#cbd5e1;border:1px solid #2a3441}} .藍{{background:linear-gradient(135deg,#2962ff,#82b1ff);color:#fff}} .紅{{background:linear-gradient(135deg,#d50000,#ff5252);color:#fff}} .白{{background:#fff;color:#000;border:2px dashed #94a3b8}}
...
.rule-box{{background:#1e293b;border:1px solid #334155;border-radius:10px;padding:12px;margin:14px 0;font-size:11px;line-height:1.8}} .ver{{background:#78350f;color:#fde68a;border:1px solid #92400e;padding:3px 10px;border-radius:8px;font-size:11px;font-weight:800}}
.custom{{border:2px solid #f59e0b !important}} .custom .badge{{background:#f59e0b;color:#000}}
...
<div class="header">
<h1>🚀 MA(20/60/240) 私有版 <span class="ver">V12 自助加股</span></h1>
<div class="search"><span class="icon">🔍</span><input id="search" type="text" placeholder="搜索 (代號/名稱模糊)" /></div>
<div class="addbox"><input id="addInput" type="text" placeholder="自助加股：輸入代號如 PLTR / 0700.HK 回車" /><button class="addbtn" onclick="addStock()">+ 加股</button></div>
</div>

<div class="sub">更新: {hk_now} 香港時間 | 共<span id="count">{len(results)}</span>隻 (雲端{len(results)}+自加<span id="customCount">0</span>) | 時k含盤前後 | 點=昨→今 | MA高於現價紅 低於綠 | 自加股存在瀏覽器永久保存</div>
...
<span class="pill" style="background:#f59e0b;color:#000">⭐自加 <span id="pillCustom">0</span>隻 <a href="#" onclick="clearCustom();return false;" style="color:#000;text-decoration:underline;margin-left:6px">清空自加</a></span>
</div>
...
⬜ 白=限流<br><br>
<b>V12 自助加股說明：</b><br>
1. 在右上角輸入框輸入代號如 <b>PLTR</b>、<b>0700.HK</b>、<b>NVDA</b> 按回車或點+加股，自動在下方生成卡片，含真實MA和顏色判斷（前端直連Yahoo，含盤前後）。<br>
2. 自加的股票存在你瀏覽器localStorage，刷新還在，換電腦會消失。<br>
3. 想永久加入雲端？點「匯出代碼」把WATCHLIST複製給我，我幫你加到雲端版本。<br>
4. 支持模糊搜索，搜索框會同時搜雲端和自加的股票。<br>
<button onclick="exportCustom()" style="margin-top:8px;padding:6px 12px;border-radius:8px;border:1px solid #334155;background:#1e293b;color:#fff;cursor:pointer">📋 匯出自加清單 (給我加到雲端)</button>
</div>
...
// 自助加股邏輯
let customStocks = JSON.parse(localStorage.getItem('customStocksV12')||'[]');
const customGrid = document.getElementById('customGrid');

function saveCustom(){{ localStorage.setItem('customStocksV12', JSON.stringify(customStocks)); updateCustomCount(); }}
function updateCustomCount(){{ document.getElementById('customCount').innerText=customStocks.length; document.getElementById('pillCustom').innerText=customStocks.length; document.getElementById('count').innerText = {len(results)} + customStocks.length; }}

function clearCustom(){{ if(!confirm('清空所有自加股票？')) return; customStocks=[]; saveCustom(); customGrid.innerHTML=''; }}

function exportCustom(){{ 
  if(customStocks.length===0){{ alert('還沒有自加股票'); return; }}
  let codeStr = customStocks.map(s=>`    ("${{s.code}}", "${{s.name}}"),`).join("\\n");
  let full = `WATCHLIST 加這幾隻：\\n${{codeStr}}\\n\\n把這段發我，我幫你加到雲端版`;
  navigator.clipboard.writeText(full).then(()=>alert('已複製到剪貼簿！\\n\\n'+full)).catch(()=>prompt('複製這段發我：', full));
}}
...
function rsiCalc(closes, period=14){{ if(closes.length<period+1) return null; let gains=0,losses=0; for(let i=1;i<=period;i++){{ let diff=closes[closes.length-period-1+i]-closes[closes.length-period-2+i]; if(diff>0) gains+=diff; else losses+=-diff; }} gains/=period; losses/=period; if(losses===0) return 100; let rs=gains/losses; return 100-(100/(1+rs)); }}

async function fetchYahoo(ticker, range, interval, includePrePost){{ 
  let url=`https://query1.finance.yahoo.com/v8/finance/chart/${{encodeURIComponent(ticker)}}?range=${{range}}&interval=${{interval}}&includePrePost=${{includePrePost}}&corsDomain=finance.yahoo.com`;
  // 用代理避免CORS，部分瀏覽器直連可通，失敗則用代理
  try{{
    let r=await fetch(url); if(!r.ok) throw new Error('yahoo fail'); return await r.json();
  }}catch(e){{
    // 代理備用
    let proxy=`https://corsproxy.io/?${{encodeURIComponent(url)}}`;
    let r=await fetch(proxy); return await r.json();
  }}
}}

async function addStock(tickerInput){{ 
  let ticker = (tickerInput||document.getElementById('addInput').value||'').trim().toUpperCase();
  if(!ticker) return;
  if(customStocks.find(s=>s.code===ticker)){{ alert(ticker+' 已加過'); return; }}
  document.getElementById('addInput').value='';
  let cardId='custom-'+ticker;
  customGrid.insertAdjacentHTML('beforeend', `<div id="${{cardId}}" class="card 白 custom" data-code="${{ticker.toLowerCase()}}" data-name="${{ticker.toLowerCase()}}"><div class="left"><div class="top"><b>${{ticker}}</b> 載入中... <span class="dots"><span class="dot 白"></span><span class="arrow">→</span><span class="dot 白"></span></span></div><div class="mid"><span class="price">抓取Yahoo中...</span></div><div class="ma-box"><div class="ma-row"><span class="ma-w">時k含盤前後 prepost=True 抓取中...</span></div></div></div><div class="badge">白</div></div>`);
  
  try{{
    let dayData = await fetchYahoo(ticker,'1y','1d',false);
    let hourData = await fetchYahoo(ticker,'2mo','60m',true);
    let dayResult = dayData.chart?.result?.[0];
    let hourResult = hourData.chart?.result?.[0];
    if(!dayResult || !dayResult.indicators?.quote?.[0]?.close) throw new Error('找不到 '+ticker);
    ...
    let name = dayResult.meta?.longName || dayResult.meta?.shortName || ticker;
    // 保存
    ...
    let rsiHtml='';
    if(rsi!==null){{ if(rsi>=70) rsiHtml=`<span class='rsi-inline rsi-green'>RSI14(${{rsi.toFixed(0)}})</span>`; else if(rsi<=30) rsiHtml=`<span class='rsi-inline rsi-red'>RSI14(${{rsi.toFixed(0)}})</span>`; }}
    ...
    <div class="card ${{today}} custom" id="${{cardId}}" data-code="${{ticker.toLowerCase()}}" data-name="${{name.toLowerCase()}} ${{ticker.toLowerCase()}}" data-custom="1">
      <div class="left">
        <div class="top"><b>${{ticker}}</b> ${{name}} <span class="dots"><span class="dot ${{yesterday}}"></span><span class="arrow">→</span><span class="dot ${{today}}"></span></span> ${{rsiHtml}} <span style="font-size:10px;opacity:0.7;cursor:pointer" onclick="removeCustom('${{ticker}}')">❌</span></div>
        <div class="mid"><span class="price">$${{curr.toFixed(2)}}</span> <span class="pct ${{pctCls}}">${{sign}}${{pct.toFixed(2)}}%</span></div>
        <div class="ma-box">${{dRow}}${{hRow}}</div>
      </div>
      <div class="badge">${{today}}</div>
    </div>`;
  }}catch(e){{
    console.error(e);
    document.getElementById(cardId).outerHTML = `<div class="card 白 custom" id="${{cardId}}" data-code="${{ticker.toLowerCase()}}" data-name="${{ticker.toLowerCase()}}"><div class="left"><div class="top"><b>${{ticker}}</b> 抓取失敗 <span class="dots"><span class="dot 白"></span><span class="arrow">→</span><span class="dot 白"></span></span> <span style="font-size:10px;cursor:pointer" onclick="removeCustom('${{ticker}}')">❌</span></div><div class="mid"><span class="price">失敗: ${{e.message}}</span></div><div class="ma-box"><div class="ma-row"><span class="ma-w">請檢查代號是否正確，如 0700.HK / AAPL</span></div></div></div><div class="badge">白</div></div>`;
  }}
}}

function removeCustom(ticker){{ customStocks = customStocks.filter(s=>s.code!==ticker); saveCustom(); let el=document.getElementById('custom-'+ticker); if(el) el.remove(); }}

document.getElementById('addInput').addEventListener('keydown', e=>{{ if(e.key==='Enter') addStock(); }});

// 載入已保存的自加股票
(async ()=>{{ updateCustomCount(); for(let s of customStocks){{ await addStock(s.code); await new Promise(r=>setTimeout(r,800)); }} }})();
