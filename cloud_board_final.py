...
for code,name in WATCHLIST:
    try:
        ...
        # V55 關鍵：時K MA包含盤前盤後 prepost=True，不再between_time過濾，盤後停在20:00最後一根去算MA20/60/240
        df_60m_reg = df_60m_all if not df_60m_all.empty else df_day
        
        # V55 curr改實時價：盤中一直跳動
        try:
            # 先試取yfinance fast_info實時價（最快，tick級）
            ...
            curr = float(df_60m_all['Close'].iloc[-1]) if not df_60m_all.empty else float(df_day['Close'].iloc[-1])
        # V55 修漲跌幅：用美股交易日(EDT)判斷昨天，避免HK日期跨日導致前天
        try:
...
<div class="sub" style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">更新: __UPDATE_TIME__ 香港時間 | 共<span id="count">__COUNT__</span>隻 自加<span id="customCount">0</span>隻 置頂<span id="pinCount">0</span>隻 |
<button id="filterUpBtn" class="export-btn" style="padding:4px 10px;border-radius:8px;font-size:12px">漲</button>
<button id="filterDownBtn" class="export-btn" style="padding:4px 10px;border-radius:8px;font-size:12px;background:#7f1d1d !important">跌</button>
<button id="filterAllBtn" class="export-btn" style="padding:4px 10px;border-radius:8px;font-size:12px;background:#334155 !important">全部</button>
</div>
