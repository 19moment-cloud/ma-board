...
for code,name in WATCHLIST:
    try:
        ...
        # V35 關鍵：時K MA包含盤前盤後，不再between_time過濾
        # 盤後結束停在20:00最後一根
        ...
        try:
            ...
            rsi14=None
        try:
            # CCI 實時：跟RSI一樣以當前價為Close，並用當天盤中最高最低更新High/Low
            df_rt = df_day.copy()
            # 今天日期
            ...
            try:
                if not df_60m_all.empty and today_dt is not None:
                    # 當天所有60m K線（含盤前盤後）
                    today_bars = df_60m_all[df_60m_all.index.date == today_dt] if hasattr(df_60m_all.index, 'date') else df_60m_all
            ...
            # High 取 max(原日K High, 今天盤中最高, curr)
            if 'High' in df_rt.columns:
                candidates = [float(df_rt.iloc[last_idx]['High'])]
                if today_high is not None: candidates.append(today_high)
                candidates.append(curr)
                df_rt.iloc[last_idx, df_rt.columns.get_loc('High')] = max(candidates)
            if 'Low' in df_rt.columns:
                candidates = [float(df_rt.iloc[last_idx]['Low'])]
                if today_low is not None: candidates.append(today_low)
                candidates.append(curr)
                df_rt.iloc[last_idx, df_rt.columns.get_loc('Low')] = min(candidates)
...
<button id="saveAllBtn" style="padding:8px 14px;border-radius:8px;border:none;background:#3b82f6;color:#fff;font-weight:800;cursor:pointer;margin-left:8px">💾 保存卡片+排序</button>
<button id="importAllBtn" style="padding:8px 14px;border-radius:8px;border:none;background:#10b981;color:#fff;font-weight:800;cursor:pointer;margin-left:8px">📥 匯入卡片+排序</button>
...
(function(){
  ...
  function initLock(){
    ...
    if(pw){
      pw.addEventListener('keydown', function(e){ if(e.key==='Enter'){ e.preventDefault(); doUnlock(); } });
      try{ pw.focus(); }catch(e){}
    }
    try{ if(sessionStorage.getItem('ma_pw')==='ok'){ var lock=document.getElementById('lock'); if(lock) lock.style.display='none'; } }catch(e){}
  }
...
function saveLayout(){ var order={}; ['grid','pinnedGrid','grid-hk','grid-index','grid-waiting'].forEach(function(id){ var el=document.getElementById(id); if(!el) return; order[id]=Array.from(el.children).map(function(c){return c.getAttribute('data-ticker')}).filter(Boolean); }); localStorage.setItem(getLayoutKey(), JSON.stringify(order)); 
  // 同步置頂順序到 pinnedStocks
  if(order['pinnedGrid']){
    pinnedStocks = order['pinnedGrid'].map(function(t){return t.toUpperCase();});
...
function restoreLayout(){ try{ var orderStr = localStorage.getItem(getLayoutKey()); if(!orderStr){ var oldKeys = ["layoutOrderV34","layoutOrderV33","layoutOrderV32","layoutOrderV31","layoutOrderV30"]; for(var i=0;i<oldKeys.length;i++){ var d = localStorage.getItem(oldKeys[i]); if(d){ orderStr = d; localStorage.setItem(getLayoutKey(), d); break; } } } if(!orderStr) return; var order=JSON.parse(orderStr); 
  // 先恢復默認等區域
  Object.keys(order).forEach(function(gridId){ if(gridId==='pinnedGrid') return; var grid=document.getElementById(gridId); if(!grid) return; var map={}; document.querySelectorAll('#grid .card, #grid-hk .card, #grid-index .card, #grid-waiting .card').forEach(function(c){ if(!c.classList.contains('pinned-clone')) map[c.getAttribute('data-ticker').toUpperCase()]=c; }); order[gridId].forEach(function(t){ var card=map[t.toUpperCase()]; if(card) grid.appendChild(card); }); });
  // 再恢復置頂區順序
  if(order['pinnedGrid'] && order['pinnedGrid'].length>0){
    var pinnedGrid=document.getElementById('pinnedGrid');
    var existingMap={};
    pinnedGrid.querySelectorAll('.card').forEach(function(c){ existingMap[c.getAttribute('data-ticker').toUpperCase()]=c; });
    // 也從其他區域找可能的卡片用於恢復
    document.querySelectorAll('#grid .card, #grid-hk .card, #grid-index .card, #grid-waiting .card').forEach(function(c){ var tk=c.getAttribute('data-ticker').toUpperCase(); if(!existingMap[tk]) existingMap[tk]=c; });
    order['pinnedGrid'].forEach(function(t){
      var card=existingMap[t.toUpperCase()];
      if(card){
        var clone=card.cloneNode(true);
        if(!card.classList.contains('pinned-clone')){
          clone.classList.add('pinned-clone');
          var star=clone.querySelector('.star'); if(star) star.textContent='★';
        }
        pinnedGrid.appendChild(clone);
        var skBox=clone.querySelector('.skdj-box'); if(skBox) initSkdjBox(skBox);
      }
    });
    // 更新 pinnedStocks 為拖動後的順序
    pinnedStocks = order['pinnedGrid'].map(function(t){return t.toUpperCase();});
    localStorage.setItem(getPinKey(), JSON.stringify(pinnedStocks));
  }
  }catch(e){ console.log('restore err', e); } }
...
document.getElementById('saveAllBtn').addEventListener('click', function(){
  var data={
    ...
    layout: JSON.parse(localStorage.getItem(getLayoutKey())||'{}'),
    pinned: JSON.parse(localStorage.getItem(getPinKey())||'[]'),
    focusDots: JSON.parse(localStorage.getItem(FOCUS_DOTS_KEY)||'{}'),
    focusMa: JSON.parse(localStorage.getItem(FOCUS_MA_KEY)||'{}')
  };
  ...
  a.href=url;
  a.download="ma_board_backup_"+new Date().toISOString().slice(0,10)+".json";
  document.body.appendChild(a);
  a.click();
  setTimeout(function(){ document.body.removeChild(a); URL.revokeObjectURL(url); }, 100);
  alert('已保存全部卡片+排序\n包含：自加'+customStocks.length+'隻，置頂'+(data.pinned.length||0)+'隻');
});
...
document.getElementById('importFile').addEventListener('change', function(e){
  ...
  reader.onload=function(ev){
    try{
      ...
      if(!data.customStocks && !data.layout && !data.pinned){ alert('檔案格式不對'); return; }
      if(!confirm('匯入後會替換當前所有卡片+排序\n自加'+(data.customStocks?data.customStocks.length:0)+'隻，置頂'+(data.pinned?data.pinned.length:0)+'隻\n確定覆蓋？')) return;
      if(data.customStocks){ localStorage.setItem(getCustomKey(), JSON.stringify(data.customStocks)); }
      if(data.layout){ localStorage.setItem(getLayoutKey(), JSON.stringify(data.layout)); }
      if(data.pinned){ localStorage.setItem(getPinKey(), JSON.stringify(data.pinned)); }
      if(data.focusDots){ localStorage.setItem(FOCUS_DOTS_KEY, JSON.stringify(data.focusDots)); }
      if(data.focusMa){ localStorage.setItem(FOCUS_MA_KEY, JSON.stringify(data.focusMa)); }
      alert('匯入成功，頁面將重載');
      location.reload();
