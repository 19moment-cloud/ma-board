...
for r in results:
    ...
    if r.get('cci14') is not None:
        v=r['cci14']
        # 100以上綠色 -100以下紅色 (標準CCI超買超賣)
        cls = "rsi-green" if v>=100 else "rsi-red" if v<=-100 else ""
        cci_html=f"<span class='rsi-inline {cls}'>CCI14 ({v:.0f})</span>"
    if is_white:
    ...
    # SKDJ 小圖容器
    skdj_box = f"<div class='skdj-box' data-k='{skdj_k_json}' data-d='{skdj_d_json}' title='SKDJ 9 6 滾輪放大'><canvas class='skdj-canvas' width='340' height='56'></canvas><div class='skdj-label'>SKDJ 9 6</div></div>" if r.get('skdj_k') and len(r.get('skdj_k'))>0 else ""
    cards+=f"""
...
/* SKDJ 小圖 */
.skdj-box{margin-top:6px;background:#111;border-radius:8px;padding:4px 6px 2px;border:1px solid #222;position:relative;height:62px;overflow:hidden;cursor:ns-resize}
.skdj-box canvas{width:100%;height:56px;display:block}
.skdj-label{position:absolute;left:6px;top:2px;font-size:9px;color:#888;background:rgba(0,0,0,0.6);padding:1px 4px;border-radius:3px}
...
<div class="header">
<h1><a href="https://hk.finance.yahoo.com/markets" target="_blank" style="text-decoration:none" title="Yahoo HK">🚀</a> MA(20/60/240) 私有版 <span class="ver">V29</span></h1>
...
<div class="sub">更新: __UPDATE_TIME__ 香港時間 | 共<span id="count">__COUNT__</span>隻 自加<span id="customCount">0</span>隻 置頂<span id="pinCount">0</span>隻 | RSI 70綠30紅 CCI 100綠-100紅 SKDJ 9 6 75/25</div>
...
function initSkdjBox(box){
  ...
  try{
    ...
    // 超買超賣背景色
    // 75以上淺紅 25以下淺綠
    ctx.fillStyle = box.classList.contains('white') ? 'rgba(255,200,200,0.3)' : 'rgba(80,20,20,0.3)';
    ctx.fillRect(0,0,w,h*0.25); // 75-100
    ctx.fillStyle = box.classList.contains('white') ? 'rgba(200,255,200,0.3)' : 'rgba(20,60,20,0.3)';
    ctx.fillRect(0,h*0.75,w,h*0.25); // 0-25
    ...
    drawLine(kSlice, '#3b82f6'); // 藍 K
    drawLine(dSlice, '#22c55e'); // 綠 D
    
    // 標金叉死叉點
    for(var i=1;i<kSlice.length;i++){
      var prevK = kSlice[i-1], prevD = dSlice[i-1];
      var curK = kSlice[i], curD = dSlice[i];
      if(prevK<=prevD && curK>curD){ // 金叉
        var x = (i/(kSlice.length-1))*w;
        var y = h - (curK/100)*h;
        ctx.fillStyle = '#22c55e';
        ctx.beginPath(); ctx.arc(x,y,3,0,Math.PI*2); ctx.fill();
      } else if(prevK>=prevD && curK<curD){ // 死叉
        var x = (i/(kSlice.length-1))*w;
        var y = h - (curK/100)*h;
    ...
    // 左下角顯示最新值
    var lastK = kSlice[kSlice.length-1];
    var lastD = dSlice[dSlice.length-1];
    ctx.fillStyle = '#3b82f6';
    ctx.font = '10px sans-serif';
    ctx.fillText(lastK.toFixed(2), 2, 10);
    ctx.fillStyle = '#22c55e';
    ctx.fillText(lastD.toFixed(2), 45, 10);
  }catch(e){ console.log('skdj draw err', e); }
}
function initAllSkdj(){
  ...
  document.querySelectorAll('.skdj-box').forEach(function(box){
    box.addEventListener('wheel', function(e){
      ...
      var cur = box._showDays || 30;
      cur += delta;
      if(cur<10) cur=10;
      var kData = JSON.parse(box.getAttribute('data-k')||'[]');
      if(cur>kData.length) cur=kData.length;
      box._showDays = cur;
      initSkdjBox(box);
    }, {passive:false});
  });
...
function getCustomKey(){ var oldKeys = ["customStocksV28","customStocksV27","customStocksV26","customStocksV25","customStocksV24"]; for(var i=0;i<oldKeys.length;i++){ var d = localStorage.getItem(oldKeys[i]); if(d && !localStorage.getItem(CUSTOM_KEY)){ localStorage.setItem(CUSTOM_KEY, d); break; } } return CUSTOM_KEY; }
...
print(f"V29 生成完成 CCI14 + SKDJ 9 6 帶小圖 60天 滾輪縮放")
