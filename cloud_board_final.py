...
import json, os, time, requests, re, math, random
...
def get_history_retry(ticker, period, interval, prepost, retries=3):
...
def calc_color(d_close, d20, d60, d240, h_close, h20, h60, h240):
    # V33 只看日k是否足夠，日k不夠才白，時k不夠用日k代替，避免全白
...
for code,name in WATCHLIST:
    try:
        ...
        df_60m_all, _ = get_history_retry(code,"60d","60m",True,2)
        if df_60m_all.empty:
            df_60m_all = df_day
            df_60m_reg = df_day
        else:
            try:
                ...
                if df_60m_reg.empty or len(df_60m_reg)<15:
        ...
        try:
            ...
            if len(df_60m_y)<15: df_60m_y = df_60m_reg.iloc[:-7]
        ...
        try:
            ...
            if len(df_60m_pre)<15: df_60m_pre = df_60m_reg.iloc[:-14]
...
for r in results:
    ...
    if is_white:
        d_row = f"<div class='ma-row'><span class='ma-w'>限流 日k不足60</span></div>"
        h_row = f"<div class='ma-row'><span class='ma-w'>請稍後重試</span></div>"
    else:
...
.count-pill.orange{border-color:#ff8a00} .count-pill.orange .dot{background:#ff8a00}
.count-pill.green{border-color:#00c853} .count-pill.green .dot{background:#00c853}
.count-pill.blue{border-color:#2962ff} .count-pill.blue .dot{background:#2962ff}
.count-pill.red{border-color:#ef4444} .count-pill.red .dot{background:#ef4444}
.count-pill.gray{border-color:#94a3b8} .count-pill.gray .dot{background:#94a3b8}
.count-pill.white-pill{border-color:#facc15 !important} .count-pill.white-pill .dot{background:#fff;border:1px solid #000}
...
<div class="header">
<h1><a href="https://hk.finance.yahoo.com/markets" target="_blank" style="text-decoration:none" title="Yahoo HK">🚀</a> MA(20/60/240) 私有版 <span class="ver">V33</span></h1>
...
function updateSectionCounts(){
  ...
  function renderPills(containerId, counts, isDefault){
    ...
    if(!el) return;
    var html='';
    // 默認區：橙綠藍紅灰 0也顯示，白 0不顯示，>0才顯示
    // 其他區：V33 按要求不顯示（但我們保留函數，傳空）
    if(isDefault){
  ...
  // 默認區統計 = 默認 + .HK + 指數，不含等候區
  // 白統計 = 默認 + .HK + 指數的白，不含等候
  var defaultAll = countGrid(['grid','grid-hk','grid-index']);
  var waitingWhite = countGrid(['grid-waiting']).白;
  // defaultAll.白 已經是 默認+HK+指數 的白，符合要求（不含等候）
...
function getCustomKey(){ var oldKeys = ["customStocksV32","customStocksV31","customStocksV30","customStocksV29","customStocksV28"]; for(var i=0;i<oldKeys.length;i++){ var d = localStorage.getItem(oldKeys[i]); if(d && !localStorage.getItem(CUSTOM_KEY)){ localStorage.setItem(CUSTOM_KEY, d); break; } } return CUSTOM_KEY; }
...
print(f"V33 生成完成 修白 限流重試 黃色外框 默認區統計含HK指數")
