...
/* V67 對齊三行右邊緣 */
#headerRow, #addRow, #subRow{width:100%;max-width:1200px;box-sizing:border-box}
#addRow{justify-content:flex-start}
#addRow #addInput{flex:1;max-width:100%}
#subRow #subText{flex:1;min-width:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}

</style></head><body>

<div id="lock">
<div style="position:absolute;left:12px;top:12px;background:#78350f;color:#facc15;border:1px solid #92400e;padding:3px 10px;border-radius:8px;font-size:11px;font-weight:800;box-shadow:0 0 8px rgba(250,204,21,0.5)">V67</div>
<h2>🔒 私有看板</h2>
...
function alignRows(){
  try{
    ...
    if(!header || !addRow || !subRow || !searchBox) return;
    // 第三行實際內容寬度
    var subWidth = subRow.scrollWidth;
    // 取三行中最大寬度，但不超過1200
    var maxW = Math.min(1200, Math.max(header.scrollWidth, addRow.scrollWidth, subWidth, 900));
    // 讓第二行和第三行寬度一致
    addRow.style.maxWidth = maxW + 'px';
    subRow.style.maxWidth = maxW + 'px';
    header.style.maxWidth = maxW + 'px';
    // 第一行搜索框拉寬去遷就下面兩行
    // 計算標題寬度
    var title = header.querySelector('h1');
    var titleW = title ? title.offsetWidth : 200;
    var rocket = document.getElementById('rocketTopBtn');
    var rocketW = rocket ? 48 : 0;
    var available = maxW - titleW - rocketW - 24;
    if(available > 200){
      searchBox.style.maxWidth = Math.min(600, available) + 'px';
    }
    // 第二行搜索選單縮短遷就寬度 (加股輸入框)
    ...
    if(addInput){
      ...
      if(subText){
        var subTextW = subText.offsetWidth;
        // 第二行輸入框寬度設為與第三行文字區相近
        addInput.style.maxWidth = '100%';
      }
    }
  }catch(e){ console.log('align error', e); }
}
document.addEventListener('DOMContentLoaded', function(){ setTimeout(alignRows, 500); setTimeout(alignRows, 1500); });
