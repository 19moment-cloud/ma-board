...
def get_yahoo_name(tk, code, fallback):
    ...
    try:
        ...
        if code in CN_NAME_MAP and CN_NAME_MAP[code]:
            # 對指數類保留英文
            if code.startswith('^'):
                return CN_NAME_MAP[code] if len(CN_NAME_MAP[code])<=15 else cleaned
            # 對中概股用中文
            if len(cleaned) > 2 and all(ord(c) < 128 for c in cleaned): # 全英文
...
for code,name in WATCHLIST:
    try:
        ...
        if df_day.empty or len(df_day)<65:
            results.append({"code":code,"name":name,"display":display,"today":"白","yesterday":"白","pre":"白","curr":0,"pct":0,"d20":0,"d60":0,"d240":0,"h20":0,"h60":0,"h240":0,"rsi14":None,"err":True})
            time.sleep(0.8); continue
        df_60m, _ = get_history(code,"60d","60m",True)
        if df_60m.empty:
            results.append({"code":code,"name":name,"display":display,"today":"白","yesterday":"白","pre":"白","curr":float(df_day['Close'].iloc[-1]),"pct":0,"d20":calc_ma(df_day['Close'],20),"d60":calc_ma(df_day['Close'],60),"d240":calc_ma(df_day['Close'],240),"h20":0,"h60":0,"h240":0,"rsi14":None,"err":True})
            time.sleep(0.8); continue
        ...
        results.append({"code":code,"name":name,"display":display,"today":today,"yesterday":yesterday,"pre":pre,"curr":round(curr,2),"pct":round(pct,2),"d20":round(d20_t,2),"d60":round(d60_t,2),"d240":round(d240_t,2),"h20":round(h20_t,2),"h60":round(h60_t,2),"h240":round(h240_t,2),"rsi14":rsi14,"err":False})
        time.sleep(0.8)
    except Exception as e:
        print(f"{code} err {e}")
        results.append({"code":code,"name":name,"display":format_display(code, CN_NAME_MAP.get(code, name)),"today":"白","yesterday":"白","pre":"白","curr":0,"pct":0,"d20":0,"d60":0,"d240":0,"h20":0,"h60":0,"h240":0,"rsi14":None,"err":True})
...
html_content = """<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
...
<div class="header">
<h1><a href="https://hk.finance.yahoo.com/markets" target="_blank" style="text-decoration:none" title="Yahoo HK">🚀</a> MA(20/60/240) 私有版 <span class="ver">V21</span></h1>
<div class="search"><span class="icon">🔍</span><input id="search" type="text" placeholder="搜索代號/名稱模糊" /></div>
</div>

<div class="addRow"><input id="addInput" type="text" placeholder="加股：輸入 TSL 自動出現選單" autocomplete="off"/><div id="yahooSuggest"></div><button class="addbtn" id="addBtn">+ 加股</button><button class="addbtn" id="clearBtn" style="background:#1e293b;color:#fff;border:1px solid #334155">清空自加</button></div>
...
(function(){
  ...
  function doUnlock(){
    ...
    if(!v){ msg.textContent="請輸入密碼"; alert("請輸入密碼"); return; }
    if(v === REAL_PW){
...
// ===== 卡片邏輯 =====
var focusDots = JSON.parse(localStorage.getItem('focusDotsV21')||localStorage.getItem('focusDotsV20')||'{}');
var focusMa = JSON.parse(localStorage.getItem('focusMaV21')||localStorage.getItem('focusMaV20')||'{}');

document.addEventListener('click', function(e){
  var dots = e.target.closest('.dots');
  if(dots){ dots.classList.toggle('white'); var t=dots.getAttribute('data-ticker'); if(t){ if(dots.classList.contains('white')) focusDots[t]=1; else delete focusDots[t]; localStorage.setItem('focusDotsV21', JSON.stringify(focusDots)); } return; }
  var mbox = e.target.closest('.ma-box');
  if(mbox){ mbox.classList.toggle('white'); var t=mbox.getAttribute('data-ticker'); if(t){ if(mbox.classList.contains('white')) focusMa[t]=1; else delete focusMa[t]; localStorage.setItem('focusMaV21', JSON.stringify(focusMa)); } return; }
...
function saveLayout(){ var order={}; ['grid','pinnedGrid','grid-hk','grid-index','grid-waiting'].forEach(function(id){ var el=document.getElementById(id); if(!el) return; order[id]=Array.from(el.children).map(function(c){return c.getAttribute('data-ticker')}).filter(Boolean); }); localStorage.setItem('layoutOrderV21', JSON.stringify(order)); }
function initSortable(){ ['grid','pinnedGrid','grid-hk','grid-index','grid-waiting'].forEach(function(id){ var el=document.getElementById(id); if(!el) return; if(el._sortable) el._sortable.destroy(); el._sortable = new Sortable(el, { group: 'shared', animation:150, ghostClass:'sortable-ghost', chosenClass:'sortable-chosen', onEnd: function(){ saveLayout(); renderPins(); }}); }); }
function restoreLayout(){ try{ var order=JSON.parse(localStorage.getItem('layoutOrderV21')||localStorage.getItem('layoutOrderV20')||'{}'); Object.keys(order).forEach(function(gridId){ var grid=document.getElementById(gridId); if(!grid) return; var map={}; document.querySelectorAll('.card').forEach(function(c){ map[c.getAttribute('data-ticker').toUpperCase()]=c; }); order[gridId].forEach(function(t){ var card=map[t.toUpperCase()]; if(card) grid.appendChild(card); }); }); }catch(e){} }

var pinnedStocks = JSON.parse(localStorage.getItem('pinnedV21')||localStorage.getItem('pinnedV20')||'[]');
function togglePin(ticker){
  ...
  localStorage.setItem('pinnedV21', JSON.stringify(pinnedStocks));
...
var customStocks = JSON.parse(localStorage.getItem('customStocksV21')||localStorage.getItem('customStocksV20')||'[]');
function saveCustom(){ localStorage.setItem('customStocksV21', JSON.stringify(customStocks)); document.getElementById('customCount').innerText=customStocks.length; }
...
function addStock(input){
  ...
  var disp='('+ticker+')'+name; if(disp.length>32) disp=disp.slice(0,31)+'…';
...
document.getElementById('addInput').addEventListener('keydown', function(e){ if(e.key==='Enter'){ var box=document.getElementById('yahooSuggest'); if(box.style.display!=='none' && box.firstChild) box.firstChild.click(); else addStock(); } });
...
document.getElementById('addInput').addEventListener('input', function(e){
  ...
  yTimer=setTimeout(function(){
    ...
    var attempt=0;
    function doAttempt(){
      if(attempt>=proxies.length){ yahooSuggest.style.display='none'; return; }
      tryFetch(proxies[attempt], function(data){
        if(!data || !data.quotes || data.quotes.length===0){ attempt++; doAttempt(); return; }
        // 成功
        var quotes=data.quotes;
        yahooSuggest.innerHTML='<div class="s-head"><span>股票代號</span><span>股票篩選器及更多</span></div>';
        quotes.forEach(function(it){
          ...
          var exch=it.exchange||it.exchDisp||'';
          var typeLabel=it.quoteType==='ETF'?'ETF':(it.quoteType==='EQUITY'?'股票':(exch||''));
...
(function(){
  ...
  customStocks.forEach(function(s){
    ...
    if(exists) return;
    var disp='('+s.code+')'+s.name; if(disp.length>32) disp=disp.slice(0,31)+'…';
...
html_content = html_content.replace("__UPDATE_TIME__", hk_now).replace("__COUNT__", str(len(results))).replace("__CARDS__", cards)

with open("docs/index.html","w",encoding="utf-8") as f:
    f.write(html_content)

print(f"V21 生成完成 共{len(results)}隻")
