...
def calc_ma_realtime(close_series, p, curr_price):
    """實時MA：最後一根用當前價/盤後最後1秒價代替"""
    try:
        if len(close_series) < p-1: return 0
        s = close_series.copy()
        if curr_price>0:
            s.iloc[-1] = curr_price  # 實時替換最後收盤為當前價
...
# 為了name，單獨拿info的批量可能還是需要Ticker，但我們先用WATCHLIST的fallback
results=[]
for code,name in WATCHLIST:
    try:
        ...
        # V34 實時MA：最後一根用當前價/盤後最後1秒價
        d_close_t = curr  # 當前價實時
        ...
        # RSI CCI 用實時價的close系列（最後替換為curr）
        try:
        ...
        try:
            df_rt = df_day.copy()
            df_rt.iloc[-1, df_rt.columns.get_loc('Close')] = curr
            # High Low 也要更新為包含curr
            if curr > df_rt.iloc[-1]['High']: df_rt.iloc[-1, df_rt.columns.get_loc('High')] = curr
            if curr < df_rt.iloc[-1]['Low']: df_rt.iloc[-1, df_rt.columns.get_loc('Low')] = curr
...
.rocket-btn{position:absolute;right:12px;bottom:12px;background:#f59e0b;color:#000;border:none;border-radius:8px;width:36px;height:36px;display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:18px;font-weight:900;box-shadow:0 2px 8px rgba(0,0,0,0.3)}
.rocket-btn:hover{background:#fbbf24}
</style></head><body>
...
<div class="header">
<h1><a href="https://hk.finance.yahoo.com/markets" target="_blank" style="text-decoration:none" title="Yahoo HK">🚀</a> MA(20/60/240) 私有版 <span class="ver">V34</span></h1>
...
function getCustomKey(){ var oldKeys = ["customStocksV33","customStocksV32","customStocksV31","customStocksV30","customStocksV29"]; for(var i=0;i<oldKeys.length;i++){ var d = localStorage.getItem(oldKeys[i]); if(d && !localStorage.getItem(CUSTOM_KEY)){ localStorage.setItem(CUSTOM_KEY, d); break; } } return CUSTOM_KEY; }
...
print(f"V34 批量下載 實時MA 黃粗框 火箭返回頂部")
