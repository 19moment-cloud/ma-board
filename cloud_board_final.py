...
for code,name in WATCHLIST:
    try:
        ...
        # V58 curr強制實時：盤前盤中盤後都會跳，修LI 11.7 vs 11.71，MU 1071
        curr = None
        today_high_rt = None
        today_low_rt = None
        try:
        ...
        # V58 漲跌幅統一：優先用info的regularMarketPreviousClose（最準，解決LI單隻不對）
        prev_close = None
        try:
            if 'regularMarketPreviousClose' in locals() or True:
                # inf在上面已取
...
# V58 CCI修復：用info的當天最高最低，修MU 94 vs Yahoo 141.11
        try:
...
<button id="rocketTopBtn" title="到最底部" style="position:absolute;right:12px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;font-size:24px;line-height:1;padding:6px 10px;border-radius:50%">🚀</button>
</div>
