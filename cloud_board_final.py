...
.sub{{color:#94a3b8;font-size:11px;margin:2px 0}} 
...
<div class="header">
<h1>🚀 MA(20/60/240) 私有版 <span class="ver">V14</span></h1>
<div class="search"><span class="icon">🔍</span><input id="search" type="text" placeholder="搜索代號/名稱模糊" /></div>
</div>
<div class="addRow"><input id="addInput" type="text" placeholder="加股：輸入 PLTR / 0700 / 0700.HK / 9988.HK 回車 (模糊支持)" /><button class="addbtn" onclick="addStock()">+ 加股</button><button class="addbtn" style="background:#1e293b;color:#fff;border:1px solid #334155" onclick="clearCustom()">清空自加</button></div>

<div class="sub">更新: {hk_now} 香港時間 | 共<span id="count">{len(results)}</span>隻</div>
<div class="sub">自加<span id="customCount">0</span>隻 置頂<span id="pinCount">0</span>隻 | 點右上角☆置頂 | 加股後匯出給我加到雲端</div>
...
// 置頂
let pinnedStocks = JSON.parse(localStorage.getItem('pinnedV14')||'[]');
...
function togglePin(ticker){{
  ...
  localStorage.setItem('pinnedV14', JSON.stringify(pinnedStocks));
...
// 自加 - 新策略：不前端抓Yahoo，避免失敗，加入清單等雲端算
let customStocks = JSON.parse(localStorage.getItem('customStocksV14')||'[]');
const customGrid=document.getElementById('customGrid');
function saveCustom(){{ localStorage.setItem('customStocksV14', JSON.stringify(customStocks)); document.getElementById('customCount').innerText=customStocks.length; document.getElementById('count').innerText = {len(results)} + customStocks.length; }}
function clearCustom(){{ if(!confirm('清空所有自加？')) return; customStocks=[]; saveCustom(); customGrid.innerHTML=''; renderPins(); }}
function exportCustom(){{ if(customStocks.length===0){{ alert('還沒有自加股票'); return; }} let codeStr=customStocks.map(s=>`    ("${{s.code}}", "${{s.name||s.code}}"),`).join("\\n"); let full=`自加清單 共${{customStocks.length}}隻：\\n${{codeStr}}\\n\\n把這段發給我，我幫你加到雲端版，下次自動更新`; navigator.clipboard.writeText(full).then(()=>alert('已複製！\\n\\n'+full)).catch(()=>prompt('複製這段發我：', full)); }}

function normalizeTicker(input){{
  input=input.trim().toUpperCase();
  if(!input) return null;
  // 支持中文？先去掉中文，保留代號
  // 模糊：0700 -> 0700.HK, 4位數字 -> .HK
  if(/^\\d{{3,5}}$/.test(input)) return input+'.HK';
  return input;
}}
...
async function addStock(input){{
  ...
  // 顯示等待雲端卡片，不嘗試前端Yahoo，避免失敗
  customGrid.insertAdjacentHTML('beforeend', `
  <div class="card 白 custom" id="${{cardId}}" data-code="${{ticker.toLowerCase()}}" data-name="${{name.toLowerCase()}} ${{ticker.toLowerCase()}}" data-ticker="${{ticker}}">
    <div class="left">
      <div class="top"><b>${{ticker}}</b> ${{name}} <span class="dots"><span class="dot 白"></span><span class="arrow">→</span><span class="dot 白"></span></span> <span style="font-size:10px;cursor:pointer" onclick="removeCustom('${{ticker}}')">❌</span></div>
      <div class="mid"><span class="price">已加入自加清單</span> <span class="pct">等待雲端</span></div>
      <div class="ma-box"><div class="ma-row"><span class="ma-w">已保存到瀏覽器，下次雲端更新(5分)後自動算MA</span></div><div class="ma-row"><span class="ma-w">點下方「匯出」發我，我幫你加到雲端永久版</span></div></div>
    </div>
    <div class="star" onclick="togglePin('${{ticker}}')">☆</div>
  </div>`);
  renderPins(); updateStars();
  // 如果雲端data.json已包含該股票，自動替換
  try{{
    let resp=await fetch('data.json'); let j=await resp.json();
    let found=j.results.find(r=>r.code.toUpperCase()===ticker.toUpperCase());
    if(found){{ 
      // 找到雲端已有，直接渲染真實卡片
      let r=found;
      ...
      <div class="card ${{r.today}} custom" id="${{cardId}}" data-code="${{r.code.toLowerCase()}}" data-name="${{r.name.toLowerCase()}} ${{r.code.toLowerCase()}}" data-ticker="${{r.code}}">
        <div class="left"><div class="top"><b>${{r.code}}</b> ${{r.name}} ${{dots}} ${{rsiHtml}} <span style="font-size:10px;cursor:pointer" onclick="removeCustom('${{r.code}}')">❌</span></div><div class="mid"><span class="price">$${{r.curr}}</span> <span class="pct ${{pctCls}}">${{sign}}${{r.pct.toFixed(2)}}%</span></div><div class="ma-box">${{dRow}}${{hRow}}</div></div><div class="star ${{isPinned(r.code)?'pinned':''}}" onclick="togglePin('${{r.code}}')">${{isPinned(r.code)?'★':'☆'}}</div>
      </div>`;
    }}
  }}catch(e){{}}
}}

function removeCustom(ticker){{ customStocks=customStocks.filter(s=>s.code!==ticker.toUpperCase()); pinnedStocks=pinnedStocks.filter(s=>s!==ticker.toUpperCase()); localStorage.setItem('pinnedV14',JSON.stringify(pinnedStocks)); saveCustom(); let el=document.getElementById('custom-'+ticker.replace(/\\./g,'-')); if(el) el.remove(); renderPins(); }}
...
(function(){{ 
  ...
  customStocks.forEach(s=>{{
    ...
    customGrid.insertAdjacentHTML('beforeend', `
    <div class="card 白 custom" id="${{cardId}}" data-code="${{s.code.toLowerCase()}}" data-name="${{s.name.toLowerCase()}} ${{s.code.toLowerCase()}}" data-ticker="${{s.code}}">
      <div class="left"><div class="top"><b>${{s.code}}</b> ${{s.name}} <span class="dots"><span class="dot 白"></span><span class="arrow">→</span><span class="dot 白"></span></span> <span style="font-size:10px;cursor:pointer" onclick="removeCustom('${{s.code}}')">❌</span></div><div class="mid"><span class="price">已加入自加清單</span> <span class="pct">等待雲端</span></div><div class="ma-box"><div class="ma-row"><span class="ma-w">刷新後等待雲端更新(5分)自動算MA，點匯出發我加永久版</span></div></div></div><div class="star" onclick="togglePin('${{s.code}}')">☆</div></div>`);
  }});
  renderPins(); updateStars();
  // 嘗試用雲端data.json替換自加卡片為真實數據
