...
for code,name in WATCHLIST:
    try:
        ...
        # V57 關鍵：時K MA包含盤前盤後 prepost=True，不再between_time過濾，盤後停在20:00最後一根去算MA20/60/240
        df_60m_reg = df_60m_all if not df_60m_all.empty else df_day
        
        # V57 curr強制實時：盤前盤中盤後都會跳，修LI 11.7 vs 11.71，MU 1071
        curr = None
        today_high_rt = None
        today_low_rt = None
        try:
...
# V57 CCI修復：用info的當天最高最低，修MU 94 vs Yahoo 141.11
        try:
        ...
        try:
            ...
            ny_today = _dt.datetime.now(NY_TZ).date()
            hk_today = _dt.datetime.now(HK_TZ).date()
            df_rt = df_day.copy()
            today_high = today_high_rt
            today_low = today_low_rt
            try:
                if today_high is None and not df_60m_all.empty:
                    for d in [hk_today, ny_today]:
                        tb = df_60m_all[df_60m_all.index.date == d]
                        if not tb.empty and 'High' in tb.columns:
                            th = float(tb['High'].max())
                            tl = float(tb['Low'].min()) if 'Low' in tb.columns else None
                            if today_high is None or th > today_high:
                                today_high = th
                            if tl is not None and (today_low is None or tl < today_low):
                                today_low = tl
                if today_high is None and not df_day.empty and df_day.index[-1].date() in [hk_today, ny_today]:
                    if 'High' in df_day.columns:
                        today_high = float(df_day['High'].iloc[-1])
                    if 'Low' in df_day.columns:
                        today_low = float(df_day['Low'].iloc[-1])
            except:
                pass
            last_idx = -1
            orig_h = float(df_rt.iloc[last_idx]['High']) if 'High' in df_rt.columns else curr
            orig_l = float(df_rt.iloc[last_idx]['Low']) if 'Low' in df_rt.columns else curr
            df_rt.iloc[last_idx, df_rt.columns.get_loc('Close')] = curr
            if 'High' in df_rt.columns:
                hc = [orig_h, curr]
                if today_high is not None: hc.append(today_high)
                df_rt.iloc[last_idx, df_rt.columns.get_loc('High')] = max(hc)
            if 'Low' in df_rt.columns:
                lc = [orig_l, curr]
                if today_low is not None: lc.append(today_low)
                df_rt.iloc[last_idx, df_rt.columns.get_loc('Low')] = min(lc)
...
document.addEventListener('DOMContentLoaded', function(){
  function updateCounts(){
    ...
    var cuEl = document.getElementById('customCount');
    var pEl = document.getElementById('pinCount');
  }
