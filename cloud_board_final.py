...
<input id="pw" type="password" placeholder="請輸入密碼" autocomplete="off" style="padding:14px 18px;border-radius:12px;border:1px solid #334155;background:#1e293b;color:#fff;font-size:18px;text-align:center;margin:14px;width:240px" onkeydown="if(event.key==='Enter') doUnlock()"/>
...
function doUnlock(){
  ...
  var v=pwEl? (pwEl.value||'').trim() : '';
  if(!v){ if(msg) msg.textContent='請輸入密碼'; return; }
  if(v===REAL_PW){
...
(function(){
  try{
    if(sessionStorage.getItem('ma_pw')==='ok'){
  ...
  document.addEventListener('keydown', function(e){
    if(e.key==='Enter'){
      var lock=document.getElementById('lock');
      if(lock && lock.style.display!=='none') doUnlock();
    }
  });
})();
var LAYOUT_KEY = "ma_board_layout_permanent"; = "ma_board_layout_permanent"; var PIN_KEY = "ma_board_pins_permanent"; var FOCUS_DOTS_KEY = "ma_board_focus_dots"; var FOCUS_MA_KEY = "ma_board_focus_ma"; var CUSTOM_KEY = "ma_board_custom_permanent";
...
document.getElementById('saveAllBtn').addEventListener('click', function(){
  var data={ version:"V44", exportTime: new Date().toISOString(), customStocks: customStocks, layout: JSON.parse(localStorage.getItem(getLayoutKey())||'{}'), pinned: JSON.parse(localStorage.getItem(getPinKey())||'[]'), focusDots: JSON.parse(localStorage.getItem(FOCUS_DOTS_KEY)||'{}'), focusMa: JSON.parse(localStorage.getItem(FOCUS_MA_KEY)||'{}') };
