...
<div id="lock">
<div style="position:absolute;left:12px;top:12px;background:#78350f;color:#fde68a;border:1px solid #92400e;padding:3px 10px;border-radius:8px;font-size:11px;font-weight:800">V60</div>
<h2>🔒 私有看板</h2>
...
function openAdvancedChart(ticker){
  try{
    ...
    var url = 'https://hk.finance.yahoo.com/chart/' + encodeURIComponent(t) + h;
    window.open(url, '_blank');
  }catch(e){ window.open('https://hk.finance.yahoo.com/chart/' + encodeURIComponent(ticker), '_blank'); }
}
document.addEventListener('dblclick', function(e){
  var card = e.target.closest('.card');
  if(!card) return;
  var ticker = card.getAttribute('data-ticker') || card.getAttribute('data-code');
