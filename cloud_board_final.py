...
for code,name in WATCHLIST:
    try:
        ...
        # V56 關鍵：時K MA包含盤前盤後 prepost=True，不再between_time過濾，盤後停在20:00最後一根去算MA20/60/240
        df_60m_reg = df_60m_all if not df_60m_all.empty else df_day
        
# V56 curr強制實時：盤前盤中盤後都會跳，修LI 11.7 vs 11.71
        curr = None
        today_high_rt = None
        today_low_rt = None
        try:
            ...
            try:
                ...
                inf = {}
            # 判斷當前處於哪個時段，用對應價格
            # info字段：currentPrice, regularMarketPrice, preMarketPrice, postMarketPrice, regularMarketDayHigh/Low
            if inf:
                # 取當天最高最低（Yahoo日K當天High就是盤前盤中盤後最高）
                if 'regularMarketDayHigh' in inf and inf['regularMarketDayHigh']:
                    today_high_rt = float(inf['regularMarketDayHigh'])
                if 'regularMarketDayLow' in inf and inf['regularMarketDayLow']:
                    today_low_rt = float(inf['regularMarketDayLow'])
                # 盤前
                if inf.get('preMarketPrice') and inf.get('preMarketState') != None:
                    # preMarketPrice存在就用
                    if float(inf.get('preMarketPrice',0))>0:
                        # 如果是盤前時段(美東4-9:30)，用盤前價
                        curr = float(inf['preMarketPrice'])
                # 盤後
                if curr is None and inf.get('postMarketPrice'):
                    if float(inf.get('postMarketPrice',0))>0:
                        # 盤後有價就用盤後價（像LI 11.71）
                        # 判斷postMarket時間是今天
        ...
        # V56 修漲跌幅：用美股交易日(EDT)判斷昨天，避免HK日期跨日導致前天
        try:
...
<div class="sub" id="subRow" style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;width:100%;max-width:1200px;box-sizing:border-box">更新: __UPDATE_TIME__ 香港時間 | 共<span id="count">__COUNT__</span>隻 自加<span id="customCount">0</span>隻 置頂<span id="pinCount">0</span>隻 |
<button id="filterUpBtn" style="padding:4px 12px;border-radius:8px;font-size:12px;font-weight:800;border:none;cursor:pointer;background:#16a34a;color:#fff">漲0隻</button>
<button id="filterDownBtn" style="padding:4px 12px;border-radius:8px;font-size:12px;font-weight:800;border:none;cursor:pointer;background:#dc2626;color:#fff">跌0隻</button>
<button id="filterAllBtn" class="export-btn" style="padding:4px 12px;border-radius:8px;font-size:12px">全部</button>
</div>
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  function filterCards(type){
    document.querySelectorAll('.card').forEach(function(c){
      ...
      var txt = pctEl.textContent.trim();
      var isUp = txt.includes('+') || (!txt.includes('-') && !txt.includes('限流') && parseFloat(txt)>0);
      // pct-up / pct-down class更準
      if(pctEl.classList.contains('pct-up')) isUp = true;
      if(pctEl.classList.contains('pct-down')) isUp = false;
      if(type==='up') c.style.display = isUp ? '' : 'none';
      else if(type==='down') c.style.display = !isUp && !pctEl.textContent.includes('限流') ? '' : 'none';
