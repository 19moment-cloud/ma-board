...
def get_yahoo_name(tk, fallback_code, fallback_name):
    try:
        ...
        if not long_name or long_name.lower()=='none': long_name = fallback_name
        if '(' in long_name and ')' in long_name: return long_name
...
for code,name in WATCHLIST:
    try:
        ...
        if df_day.empty or len(df_day)<65:
            results.append({"code":code,"name":name,"display":get_yahoo_name(tk,code,name),"today":"白","yesterday":"白","pre":"白","curr":0,"pct":0,"d20":0,"d60":0,"d240":0,"h20":0,"h60":0,"h240":0,"rsi14":None})
            time.sleep(1.0); continue
        df_60m, _ = get_history(code,"60d","60m",True)
        if df_60m.empty:
            results.append({"code":code,"name":name,"display":get_yahoo_name(tk,code,name),"today":"白","yesterday":"白","pre":"白","curr":float(df_day['Close'].iloc[-1]),"pct":0,"d20":calc_ma(df_day['Close'],20),"d60":calc_ma(df_day['Close'],60),"d240":calc_ma(df_day['Close'],240),"h20":0,"h60":0,"h240":0,"rsi14":None})
            time.sleep(1.0); continue
        ...
        results.append({"code":code,"name":name,"display":get_yahoo_name(tk,code,name),"today":today,"yesterday":yesterday,"pre":pre,"curr":round(curr,2),"pct":round(pct,2),"d20":round(d20_t,2),"d60":round(d60_t,2),"d240":round(d240_t,2),"h20":round(h20_t,2),"h60":round(h60_t,2),"h240":round(h240_t,2),"rsi14":rsi14})
        time.sleep(1.0)
    except Exception as e:
        print(f"{code} err {e}")
        results.append({"code":code,"name":name,"display":f"{name} ({code})","today":"白","yesterday":"白","pre":"白","curr":0,"pct":0,"d20":0,"d60":0,"d240":0,"h20":0,"h60":0,"h240":0,"rsi14":None})
...
if os.path.exists(prev_path):
    try:
        with open(prev_path,"r",encoding="utf-8") as jf:
            for r in json.load(jf).get("results",[]):
...
for r in results:
    ...
    if r.get('rsi14') is not None:
        v=r['rsi14']
        if v>=70: rsi_html=f"<div class='rsi-line rsi-green'>RSI14 ({v:.0f})</div>"
        elif v<=30: rsi_html=f"<div class='rsi-line rsi-red'>RSI14 ({v:.0f})</div>"
        else: rsi_html=f"<div class='rsi-line'>RSI14 ({v:.0f})</div>"
    if is_white:
    ...
    display = r.get('display', f"{r['name']} ({r['code']})")
    cards+=f"""
...
<div class="addRow"><input id="addInput" type="text" placeholder="加股：輸入 TS 會自動搜 Yahoo" autocomplete="off"/><div id="yahooSuggest"></div><button class="addbtn" id="addBtn">+ 加股</button><button class="addbtn" id="clearBtn" style="background:#1e293b;color:#fff;border:1px solid #334155">清空自加</button></div>
...
<script>
// 鎖屏 - 修復版用 addEventListener
(function(){{
  const REAL_PW="{WEB_PASSWORD}";
  function unlock(){{
    const v=document.getElementById('pw').value.trim();
    if(v===REAL_PW){{ document.getElementById('lock').style.display='none'; try{{sessionStorage.setItem('ma_pw','ok');}}catch(e){{}} }}
    else{{ document.getElementById('lockMsg').innerText='密碼錯誤'; }}
  }}
...
let focusDots = JSON.parse(localStorage.getItem('focusDotsV19')||'{{}}');
let focusMa = JSON.parse(localStorage.getItem('focusMaV19')||'{{}}');

document.addEventListener('click', function(e){{
  let dots = e.target.closest('.dots');
  if(dots){{ dots.classList.toggle('white'); let t=dots.getAttribute('data-ticker'); if(t){{ if(dots.classList.contains('white')) focusDots[t]=1; else delete focusDots[t]; localStorage.setItem('focusDotsV19', JSON.stringify(focusDots)); }} return; }}
  let mbox = e.target.closest('.ma-box');
  if(mbox){{ mbox.classList.toggle('white'); let t=mbox.getAttribute('data-ticker'); if(t){{ if(mbox.classList.contains('white')) focusMa[t]=1; else delete focusMa[t]; localStorage.setItem('focusMaV19', JSON.stringify(focusMa)); }} return; }}
...
function saveLayout(){{ let order={{}}; ['grid','pinnedGrid','grid-index','grid-hk','grid-waiting'].forEach(id=>{{ let el=document.getElementById(id); if(!el) return; order[id]=Array.from(el.children).map(c=>c.getAttribute('data-ticker')).filter(Boolean); }}); localStorage.setItem('layoutOrderV19', JSON.stringify(order)); }}
function initSortable(){{ ['grid','pinnedGrid','grid-index','grid-hk','grid-waiting'].forEach(id=>{{ let el=document.getElementById(id); if(!el) return; if(el._sortable) el._sortable.destroy(); el._sortable = new Sortable(el, {{ group: 'shared', animation:150, ghostClass:'sortable-ghost', chosenClass:'sortable-chosen', onEnd: ()=>{{ saveLayout(); renderPins(); }} }}); }}); }}
function restoreLayout(){{ try{{ let order=JSON.parse(localStorage.getItem('layoutOrderV19')||'{{}}'); Object.keys(order).forEach(gridId=>{{ let grid=document.getElementById(gridId); if(!grid) return; let map={{}}; document.querySelectorAll('.card').forEach(c=>{{ map[c.getAttribute('data-ticker').toUpperCase()]=c; }}); order[gridId].forEach(t=>{{ let card=map[t.toUpperCase()]; if(card) grid.appendChild(card); }}); }}); }}catch(e){{}} }}

let pinnedStocks = JSON.parse(localStorage.getItem('pinnedV19')||'[]');
...
let customStocks = JSON.parse(localStorage.getItem('customStocksV19')||'[]');
...
function addStock(input){{
  ...
  document.getElementById('grid-waiting').insertAdjacentHTML('beforeend', `<div class="card 白 custom" id="${{id}}" data-code="${{ticker.toLowerCase()}}" data-name="${{name.toLowerCase()}} ${{ticker.toLowerCase()}}" data-ticker="${{ticker}}"><div class="left"><div class="top"><b>${{ticker}}</b> ${{name}} (${{ticker}}) <span class="dots" data-ticker="${{ticker}}"><span class="dot 白"></span><span class="arrow">→</span><span class="dot 白"></span><span class="arrow">→</span><span class="dot 白"></span></span> <span style="font-size:10px;cursor:pointer" onclick="removeCustom('${{ticker}}')">❌</span></div><div class="mid"><span class="price">已加入等候區</span></div></div><div class="star" data-pin="${{ticker}}">☆</div></div>`);
  saveLayout(); applyFocus();
}}
document.getElementById('addBtn').addEventListener('click', ()=>addStock());
function removeCustom(t){{ customStocks=customStocks.filter(s=>s.code!==t.toUpperCase()); localStorage.setItem('customStocksV19',JSON.stringify(customStocks)); saveCustom(); let el=document.getElementById('custom-'+t.replace(/\\./g,'-')); if(!el) el=document.querySelector(`[data-ticker="${{t}}"]`); if(el) el.remove(); saveLayout(); renderPins(); }}
...
document.getElementById('addInput').addEventListener('input', e=>{{
  ...
  yTimer=setTimeout(async ()=>{{
    try{{
      let r=await fetch(`https://query1.finance.yahoo.com/v1/finance/search?q=${{encodeURIComponent(q)}}&quotesCount=10&newsCount=0`);
      let d=await r.json(); let quotes=d.quotes||[];
      if(!quotes.length){{ box.style.display='none'; return; }}
      box.innerHTML=''; quotes.forEach(it=>{{ let div=document.createElement('div'); div.className='s-item'; div.innerHTML=`<span><b>${{it.symbol}}</b> ${{it.shortname||it.longname||''}}</span>`; div.onclick=()=>{{ document.getElementById('addInput').value=it.symbol+' '+(it.shortname||''); box.style.display='none'; addStock(it.symbol+' '+(it.shortname||'')); }}; box.appendChild(div); }}); box.style.display='block';
...
(function(){{
  ...
  customStocks.forEach(s=>{{
    ...
    document.getElementById('grid-waiting').insertAdjacentHTML('beforeend', `<div class="card 白 custom" id="${{id}}" data-code="${{s.code.toLowerCase()}}" data-name="${{s.name.toLowerCase()}} ${{s.code.toLowerCase()}}" data-ticker="${{s.code}}"><div class="left"><div class="top"><b>${{s.code}}</b> ${{s.name}} (${{s.code}})</div><div class="mid"><span class="price">等候區</span></div></div><div class="star" data-pin="${{s.code}}">☆</div></div>`);
  }});
  fetch('data.json').then(r=>r.json()).then(j=>{{
    j.results.forEach(r=>{{
      ...
      if(card){{
        ...
        let rsi=''; if(r.rsi14!=null) rsi=`<div class='rsi-line'>RSI14 (${{r.rsi14.toFixed(0)}})</div>`;
