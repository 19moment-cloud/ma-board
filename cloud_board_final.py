...
for code,name in WATCHLIST:
    try:
        ...
        try:
            ...
            if df_60m_y.empty:
                df_60m_y = df_60m.iloc[:-7]  # 7根60m約1天
...
for r in results:
    ...
    dots=f"<span class='dots'><span class='dot {r['yesterday']}' title='昨:{r['yesterday']}'></span><span class='arrow'>→</span><span class='dot {r['today']}' title='今:{r['today']}'></span></span>"
    rsi_inline=""
    ...
    if is_white:
        d_row = f"<div class='ma-row'><span class='ma-w'>限流未抓到</span></div>"
...
.search{{flex:1;min-width:220px;max-width:360px;position:relative}} .search input{{width:100%;padding:10px 14px 10px 36px;border-radius:12px;border:1px solid #334155;background:#1e293b;color:#fff;font-size:13px;outline:none}} .search input:focus{{border-color:#f59e0b}} .search .icon{{position:absolute;left:10px;top:50%;transform:translateY(-50%)}}
.sub{{color:#94a3b8;font-size:11px;margin-bottom:12px}} .grid{{display:grid;grid-template-columns:repeat(auto-fill,minmax(360px,1fr));gap:12px;align-items:start}}
...
<div class="header"><h1>🚀 MA(20/60/240) 私有版 <span class="ver">V11</span></h1><div class="search"><span class="icon">🔍</span><input id="search" type="text" placeholder="搜索代號或名稱，例如 TSLA 特斯拉 (模糊)" /></div></div>
<div class="sub">更新: {hk_now} 香港時間 | 共{len(results)}隻 | 時k MA含盤前後已確認 | 點=昨→今 已修復真·昨天</div>
...
⚪ 不符合以上條件。<br>
⬜ 白=限流<br><br>
<b>V11 修復：</b>1. 圓點現在是真·昨→今，不再全一致，會出現 🟠→🟢 這種變色 2. MA顏色已反轉：(價)高於現價紅字(壓力) 低於現價綠字(支撐)
</div>
<script>
