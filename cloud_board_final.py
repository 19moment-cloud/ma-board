...
for code,name in WATCHLIST:
    try:
        ...
        df_60m_all, _ = get_history(code,"60d","60m",True)
        if df_60m_all.empty:
            # 沒拿到60m就用日線代替，避免變白
            df_60m_all = df_day
            df_60m_reg = df_day
            ...
            try:
                df_60m_reg = df_60m_all.between_time('09:30','16:00')
                if df_60m_reg.empty or len(df_60m_reg)<20:
                    df_60m_reg = df_60m_all
            except:
                df_60m_reg = df_60m_all
        ...
        h20_t = calc_ma(df_60m_reg['Close'],20); h60_t = calc_ma(df_60m_reg['Close'],60); h240_t = calc_ma(df_60m_reg['Close'],240)
        h_close_t = d_close_t
        ...
        try:
            ...
            df_60m_y = df_60m_reg[df_60m_reg.index.date < today_date]
            if len(df_60m_y)<20: df_60m_y = df_60m_reg.iloc[:-7]
        ...
        try:
            ...
            df_60m_pre = df_60m_reg[df_60m_reg.index.date < y_date]
            if len(df_60m_pre)<20: df_60m_pre = df_60m_reg.iloc[:-14]
...
<div class="header">
<h1><a href="https://hk.finance.yahoo.com/markets" target="_blank" style="text-decoration:none" title="Yahoo HK">🚀</a> MA(20/60/240) 私有版 <span class="ver">V32</span></h1>
...
<div class="section"><div class="section-title" id="defaultTitle"><span style="width:4px"></span><div class="section-counts" id="defaultCounts"></div></div><div class="grid" id="grid">__CARDS__</div></div>

<div class="section"><div class="section-title">.HK<div class="section-counts" id="hkCounts"></div></div><div class="grid" id="grid-hk"></div></div>
<div class="section"><div class="section-title">指數<div class="section-counts" id="indexCounts"></div></div><div class="grid" id="grid-index"></div></div>
<div class="section"><div class="section-title">等候 (自加待建 custom.json)<div class="section-counts" id="waitingCounts"></div></div><div class="grid" id="grid-waiting"></div></div>
...
function updateSectionCounts(){
  ...
  function renderPills(containerId, counts){
    var el=document.getElementById(containerId);
    if(!el) return;
    var html='';
    if(counts.橙>0) html+='<span class="count-pill orange"><span class="dot"></span>橙 主升 <b>'+counts.橙+'隻</b></span>';
    if(counts.綠>0) html+='<span class="count-pill green"><span class="dot"></span>綠 次強 <b>'+counts.綠+'隻</b></span>';
    if(counts.藍>0) html+='<span class="count-pill blue"><span class="dot"></span>藍 弱 <b>'+counts.藍+'隻</b></span>';
    if(counts.紅>0) html+='<span class="count-pill red"><span class="dot"></span>紅 空頭 <b>'+counts.紅+'隻</b></span>';
    if(counts.灰>0) html+='<span class="count-pill gray"><span class="dot"></span>灰 震盪 <b>'+counts.灰+'隻</b></span>';
    if(counts.白>0) html+='<span class="count-pill" style="border-color:#f59e0b"><span class="dot" style="background:#fff;border:1px solid #000"></span>白 限流 <b>'+counts.白+'隻</b></span>';
    el.innerHTML=html;
  }
  var defCounts=countGrid('grid');
  renderPills('defaultCounts', defCounts);
  renderPills('hkCounts', countGrid('grid-hk'));
  renderPills('indexCounts', countGrid('grid-index'));
  renderPills('waitingCounts', countGrid('grid-waiting'));
}
...
function getCustomKey(){ var oldKeys = ["customStocksV31","customStocksV30","customStocksV29","customStocksV28","customStocksV27"]; for(var i=0;i<oldKeys.length;i++){ var d = localStorage.getItem(oldKeys[i]); if(d && !localStorage.getItem(CUSTOM_KEY)){ localStorage.setItem(CUSTOM_KEY, d); break; } } return CUSTOM_KEY; }
...
print(f"V32 生成完成 修復白屏bug 紫彩虹默認區加黃字統計")
